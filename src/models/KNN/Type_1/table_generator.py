...
def clustream(table, idx, code, lookup, num_features, num_classes, num_depth, depth_num, center, width, border_max, border_min, knn_clf, division, is_return):
    cla = 0
    # if not is_return:
    is_return == False
    for f in range(num_features):
    ...
    if not_finish:
        ...
        for division in lookup:
            ...
            table, idx, is_return = clustream(table, idx, copy.deepcopy(code), lookup, num_features, num_classes,
                                   ...
                                   copy.deepcopy(new_boarder_max), copy.deepcopy(new_boarder_mim), knn_clf, division, is_return)
            depth_num -=1
...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    Ternary_Table = {}
    Ternary_Table, _, _ = clustream(Ternary_Table, 0, '', lookup, num_features, num_classes, num_depth, 0, np.zeros(num_features), np.zeros(num_features), feature_max, feature_min, knn_clf, '', False)
    print('Done')
    ...
    json.dump(Ternary_Table, open('Tables/Ternary_Table.json', 'w'), indent=4, cls=NpEncoder)
    ...
    Planter_config['p4 config']["model"] = "KNN"
