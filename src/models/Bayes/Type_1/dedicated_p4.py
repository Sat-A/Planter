...
def separate_tables(fname, config):
    with open(fname, 'a') as ingress:
        for f in range(0, config['num_features']):
            ...
            ingress.write("){\n")
            for c in range(0, config['num_classes']):
                ingress.write("        meta.prob_c" + str(c)+" = meta.prob_c" + str(c)+" + f"+str(f)+"c"+str(c) +";\n")
            ingress.write("    }\n\n")
    ...
    for f in range(num_features):
        for idx in Exact_Table['feature ' + str(f)]:
            ...
            for c in range(num_classes):
                Entry["action_params"]["f"+str(f)+"c"+str(c)] = Exact_Table['feature ' + str(f)][idx]["class "+str(c)]
            Table_entries += [Entry]

    Entry = {}
    Entry["table"] = "SwitchIngress.class_prob"
    Entry["match"] = {}
    Entry["match"]["meta.flag"] = 1
    Entry["action_name"] = "SwitchIngress.read_class_prob"
    Entry["action_params"] = {}
    for c in range(num_classes):
        Entry["action_params"]["p_c" + str(c)] = Exact_Table["class prob"]["class " + str(c)]
    Table_entries += [Entry]
...
def create_tables_Commend(fname, config):
    ...
    with open(fname, 'w') as file:
        ...
        file.write("table_add SwitchIngress.class_prob read_class_prob 1 => ")
        for c in range(num_classes):
            file.write(str(Exact_Table["class prob"]["class " + str(c)])+" ")
        file.write("\n")
