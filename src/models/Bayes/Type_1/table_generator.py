...
def resource_prediction():
    ...
    logical_dependency_memory = np.zeros(num_features+1)
    logical_dependency_memory[0] = num_classes * ( Planter_config['p4 config']["action data bits"] + 8)
    print('Exact match entries: ',np.sum(Planter_config['p4 config']["feature tbl len"]) )
    for s in range(num_features):
        logical_dependency_memory[s+1] = Planter_config['p4 config']["feature tbl len"][s]*(Planter_config['p4 config']["action data bits"]+32)

    Planter_config['resource info']['logical memory per dependency'] = logical_dependency_memory.astype(int)
    Planter_config['resource info']['total table bits'] = np.sum(logical_dependency_memory.astype(int))
    # print('The predicted number of bits usage per dependency stage:\n', logical_dependency_memory.astype(int), ', in total:', np.sum(logical_dependency_memory.astype(int)), "bits")
    json.dump(Planter_config, open(Planter_config['directory config']['work'] + '/src/configs/Planter_config.json', 'w'), indent=4, cls=NpEncoder)
    print(Planter_config['directory config']['work'] + '/src/configs/Planter_config.json is updated')
